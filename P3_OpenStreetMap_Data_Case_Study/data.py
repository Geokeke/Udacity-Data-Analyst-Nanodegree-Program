...
def shape_tag(el, tag): 
    ...
    if LOWER_COLON.match(tags['key']):
    	if tags['key'] == "name:en":
                tags['value'] = audit.update_way_names(tags['value'], audit.mapping)
        elif tags['key'] == "addr:postcode":
            if audit.check_postcode(tags['value']):
                tags['value'] = tag.attrib['v']
        tags['type'], _, tags['key'] = tags['key'].partition(':')
